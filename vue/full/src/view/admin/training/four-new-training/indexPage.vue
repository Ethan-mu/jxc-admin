  ...
  <div class="app-container doc-page-container">
    <el-card shadow="hover" style="height: 100%">
      <div slot="header">
        <el-form :inline="true" :model="search" size="mini" @submit.native.prevent>
          ...
          <el-form-item label="员工姓名">
            <el-input v-model.trim="search.employeeName" placeholder="请输入员工姓名" clearable />
          </el-form-item>
      ...
      >
        ...
        <el-table-column align="center" prop="employeeName" label="员工姓名" min-width="100" />
        ...
        <el-table-column align="center" prop="trainingDate" label="培训日期" min-width="120" />
        <el-table-column align="center" prop="trainingLocation" label="培训地点" min-width="120" />
        <el-table-column align="center" prop="lecturer" label="主讲人" min-width="100" />
      ...
      <!-- 表单弹窗 -->
      <el-dialog
        :title="dialogType === 'add' ? '新增四新技术培训记录' : '编辑四新技术培训记录'"
      ...
      >
        <el-form ref="form" :model="form" :rules="rules" label-width="120px" size="mini">
          <el-tabs v-model="activeTab">
              ...
              <el-row :gutter="20">
                <el-col :span="12">
                  <el-form-item label="员工姓名" prop="employeeName">
                    <el-input v-model.trim="form.employeeName" placeholder="请输入员工姓名" />
                  </el-form-item>
                </el-col>
                <el-col :span="12">
              ...
              <el-row :gutter="20">
                ...
                <el-col :span="12">
                  <el-form-item label="主讲人" prop="lecturer">
                    <el-input v-model.trim="form.lecturer" placeholder="请输入主讲人" />
                  </el-form-item>
                </el-col>
                <el-col :span="12">
            ...
            <el-tab-pane label="四新技术" name="technology">
              <el-row :gutter="20">
                <el-col :span="24">
                  <el-form-item label="新工艺内容" prop="newTechnology">
                    <el-input
                      v-model.trim="form.newTechnology"
                      type="textarea"
                      :rows="3"
                      placeholder="请输入新工艺内容"
                    />
                  </el-form-item>
                </el-col>
                <el-col :span="24">
                  <el-form-item label="新设备操作要点" prop="newEquipment">
                    <el-input
                      v-model.trim="form.newEquipment"
                      type="textarea"
                      :rows="3"
                      placeholder="请输入新设备操作要点"
                    />
                  </el-form-item>
                </el-col>
                <el-col :span="24">
                  <el-form-item label="新技术规范" prop="newTechnique">
                    <el-input
                      v-model.trim="form.newTechnique"
                      type="textarea"
                      :rows="3"
                      placeholder="请输入新技术规范"
                    />
                  </el-form-item>
                </el-col>
                <el-col :span="24">
                  <el-form-item label="新材料特性" prop="newMaterial">
                    <el-input
                      v-model.trim="form.newMaterial"
                      type="textarea"
                      :rows="3"
                      placeholder="请输入新材料特性"
                    />
                  </el-form-item>
                </el-col>
              </el-row>
...
import { search, add, update, del, exportTraining } from "@/api/training/fourNewTraining"
  ...
  methods: {
    ...
    refresh(reset = false) {
      ...
      search(this.searchParams)
        ...
        .catch(err => {
          console.error('获取四新技术培训列表失败', err)
    ...
    handleView(row) {
      this.$alert(`
        <div style="max-height: 500px; overflow-y: auto;">
          <h3 style="text-align: center;">${row.employeeName}的四新技术培训详情</h3>
          <h4>基本信息</h4>
          <p><strong>员工姓名：</strong>${row.employeeName}</p>
          ...
          <h4>培训信息</h4>
          <p><strong>培训日期：</strong>${row.trainingDate}</p>
          <p><strong>培训地点：</strong>${row.trainingLocation}</p>
          <p><strong>主讲人：</strong>${row.lecturer}</p>
          <p><strong>主讲内容：</strong>${row.mainContent}</p>
          <p><strong>考核成绩：</strong>${row.score}</p>
          
          <h4>四新技术</h4>
          <p><strong>新工艺内容：</strong>${row.newTechnology || '无'}</p>
          <p><strong>新设备操作要点：</strong>${row.newEquipment || '无'}</p>
          <p><strong>新技术规范：</strong>${row.newTechnique || '无'}</p>
          <p><strong>新材料特性：</strong>${row.newMaterial || '无'}</p>
        </div>
      `, '四新技术培训详情', {
    ...
    // 删除
    handleDelete(row) {
      elConfirm(`确认删除 "${row.employeeName}" 的四新技术培训记录吗？`, '删除确认')
        .then(() => {
          del(row.id)
            .then(res => {
              if (res.success) {
            ...
            .catch(err => {
              console.error('删除四新技术培训记录失败', err)
              elError('删除失败，请稍后重试')
            })
        })
    },
    ...
    handleExport() {
      exportTraining(this.searchParams)
        .then(res => {
          ...
          link.download = '四新技术培训记录.xlsx'
          link.click()
          URL.revokeObjectURL(link.href)
        })
        .catch(err => {
          console.error('导出四新技术培训记录失败', err)
          elError('导出失败，请稍后重试')
        })
    },
    ...
    submitForm() {
      this.$refs.form.validate(valid => {
        ...
        request
          ...
          .catch(err => {
            console.error(this.dialogType === 'add' ? '添加四新技术培训记录失败' : '更新四新技术培训记录失败', err)
