<template>
  <div class="doc-page-container">
    ...
    <!-- 添加/编辑对话框 -->
    <el-dialog
      :title="dialogType === 'add' ? '添加计划外培训记录' : '编辑计划外培训记录'"
...
<script>
import { search, add, update, del, exportTraining } from '@/api/training/unplannedTraining'
...
export default {
  ...
  methods: {
    ...
    // 导出功能
    handleExport() {
      this.$confirm('确认导出所有计划外培训数据吗？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
    ...
    download() {
      ...
      exportTraining(this.search).then(response => {
        const blob = new Blob([response], { type: 'application/vnd.ms-excel' })
        const link = document.createElement('a')
        link.href = window.URL.createObjectURL(blob)
        link.download = '计划外培训数据_' + timeFormat(new Date(), 'yyyy-MM-dd') + '.xlsx'
        link.click()
        loading.close()
      }).catch(() => {
        loading.close()
