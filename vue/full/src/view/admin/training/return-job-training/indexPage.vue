<template>
  <div class="app-container doc-page-container">
    <el-card shadow="hover" style="height: 100%">
      ...
      <!-- 表单弹窗 -->
      <el-dialog
        :title="dialogType === 'add' ? '新增转岗教育培训记录' : '编辑转岗教育培训记录'"
...
import { search, add, update, del, exportTraining } from "@/api/training/returnJobTraining"
  ...
  methods: {
    ...
    refresh(reset = false) {
      ...
      search(this.searchParams)
        ...
        .catch(err => {
          console.error('获取转岗教育培训列表失败', err)
    ...
    handleView(row) {
      this.$alert(`
        <div style="max-height: 500px; overflow-y: auto;">
          <h3 style="text-align: center;">${row.name}的转岗教育培训详情</h3>
          <h4>基本信息</h4>
    ...
    // 删除
    handleDelete(row) {
      elConfirm(`确认删除 "${row.name}" 的转岗教育培训记录吗？`, '删除确认')
        .then(() => {
          del(row.id)
            .then(res => {
              if (res.success) {
            ...
            .catch(err => {
              console.error('删除转岗教育培训记录失败', err)
              elError('删除失败，请稍后重试')
            })
        })
    },
    ...
    handleExport() {
      exportTraining(this.searchParams)
        .then(res => {
          ...
          link.download = '转岗教育培训记录.xlsx'
          link.click()
          URL.revokeObjectURL(link.href)
        })
        .catch(err => {
          console.error('导出转岗教育培训记录失败', err)
          elError('导出失败，请稍后重试')
        })
    },
    ...
    submitForm() {
      this.$refs.form.validate(valid => {
        ...
        request
          ...
          .catch(err => {
            console.error(this.dialogType === 'add' ? '添加转岗教育培训记录失败' : '更新转岗教育培训记录失败', err)
